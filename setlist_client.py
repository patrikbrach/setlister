    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if resp.status_code == 200:
                ...
                return {"id": row_id, "status": "error", "message": "Invalid API key (401)", "artist": artist, "date": date}
                ...
                if attempt < MAX_RETRIES:
                    ...
                    continue
                return {"id": row_id, "status": "error", "message": "Rate limit — try again later", "artist": artist, "date": date}
            ...
            if attempt < MAX_RETRIES:
                ...
                await asyncio.sleep(RETRY_DELAY * attempt)
                continue
            return {"id": row_id, "status": "error", "message": f"Network error: {e}", "artist": artist, "date": date}

    return {"id": row_id, "status": "error", "message": "Max retries reached", "artist": artist, "date": date}
